...
def eval(model, G, device, train_loader):
    ...
    for data, target in train_loader:
        ...
        for _ in range(k):
            ...
            x_adv = torch.clamp(data + args.epsilon * torch.clamp(args.enlarge_factor * adv, -1, 1), 0.0, 1.0).detach()
...
def main():
    ...
    # encoder.load_state_dict(torch.load('/data/zhijie/g_attack_pgd/encoder-epoch20.pt'))
