...
def _pgd_whitebox(model,
    ...
    for _ in range(num_steps):
        ...
        print('iter: ', _)
        for g in gradient:
            print(torch.sum(g * grad) / g.norm() / grad.norm())
        ...
        print('step: ', _, 'err pgd (white-box): ', err_pgd)

    pert = X_pgd.data - X.data
    for g in gradient:
            print(torch.sum(pert * g) / g.norm() / pert.norm())
    print('===========================')
    return err, err_pgd
...
def eval_mc_attack(model, device, test_loader):
    model.eval()
    robust_err_total = 0
    natural_err_total = 0
    ...
    for data, target in test_loader:
        data, target = data.to(device), target.to(device)
        out = model(data)
        err = (out.max(1)[1] != target).float().sum()
        print('err: ', err)

        mean = Variable(torch.zeros(data.size()).to(device), requires_grad=True)
        variance = Variable(torch.zeros(data.size()).to(device), requires_grad=True)

        optimizer = optim.Adam([mean, variance], lr=0.5)
        for s in range(args.num_steps):
            for i in range(10):
                adv_std = F.softplus(variance)
                rand_noise = torch.randn_like(data)
                ...
                logits = model(x_adv)
                loss = -F.cross_entropy(logits, target) + 0.05 * entropy
                loss.backward(retain_graph=True if i != 9 else False)
                print(s, i, entropy)

            optimizer.step()


            x_adv = torch.clamp(data + args.epsilon * torch.tanh(mean + F.softplus(variance) * torch.randn_like(data)), 0.0, 1.0)
            err_adv = (model(x_adv).max(1)[1] != target).float().sum()

            print('step: ', s, 'err adv (white-box): ', err_adv, mean.abs().mean(), F.softplus(variance).mean())
...
def main():
    ...
    eval_mc_attack(model, device, test_loader)
